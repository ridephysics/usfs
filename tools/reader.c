...
#define BARO_SAMPLESZ (BARO_RAWSZ + sizeof(uint64_t))
#define TOTAL_SAMPLESZ (MPU_SAMPLESZ + BARO_SAMPLESZ)
...
static int write_sentral_pt(struct bmp280_calib_param *calib_param, const uint8_t data[TOTAL_SAMPLESZ]) {
    ...
    uint64_t bmp_time = *((uint64_t*)&data[MPU_SAMPLESZ + BARO_RAWSZ]);
    ...
    // TODO: calibrate
    for (i = 0; i < 3; i++) {
        tmp = ((double)accel[i]) / ((double)accel_sens);
    ...
    // TODO: calibrate
    for (i = 0; i < 3; i++) {
        tmp = ((double)gyro[i]) / ((double)gyro_sens);
        if (fwrite(&tmp, sizeof(tmp), 1, stdout)!= 1) {
            return -1;
        }
    }
    ...
    // TODO: calibrate
    for (i = 0; i < 3; i++) {
        tmp = ((double)mag[i]) / (32768.0 / ((double)mag_fsr));
        if (fwrite(&tmp, sizeof(tmp), 1, stdout)!= 1) {
            return -1;
        }
    }
...
static int handle_i2c_sentral_pt(void) {
    ...
    while (keep_running) {
        ...
        *((uint64_t*)&data[MPU_SAMPLESZ + BARO_RAWSZ]) = usfs_get_us();
